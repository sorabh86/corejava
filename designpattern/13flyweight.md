...
<img src="../images/pattern/structural/Flyweight.svg" width="100%" />
...
- We start by identifying `intrinsic`(Belonging to a thing by its very nature) & `extrinsic`(Not forming an essential part of a thing, arising or originating from the outside) state of our object
...
- String pool which is maintained by JVM is also an example of flyweight. We can call the intern() method on a String object to explicitly request this String object to be interned. This method will returned a reference to already cached object if present or else will create new String in cache if not present. *Note that: String.intern() is a native method*

## Difference between Flyweight & Object Pool
- State of flyweight object is divided & client must provide part of state to it. A pooled object contains all its state encapsulated within itself.
- In a typical usage client will not change intrinsic state of flyweight instance as it is shared. Object Pool, where clients can and will change state of pooled objects.
...
- It is often difficult to find perfect candidate objects for flyweight. Graphical application benefit heavily from this pattern however a typical web application may not have a lot of use for this pattern.
